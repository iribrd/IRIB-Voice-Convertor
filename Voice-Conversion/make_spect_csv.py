...
for subdir in sorted(subdirList):
    ...
    if not os.path.exists(os.path.join(targetcsvDir, subdir)):
        os.makedirs(os.path.join(targetcsvDir, subdir))
    ...
    for fileName in sorted(fileList):
        ...
        np.savetxt(os.path.join(targetcsvDir, subdir, fileName[:-4])+".csv", S.astype(np.float32), delimiter=',')
